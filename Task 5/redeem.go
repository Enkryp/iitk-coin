...
func redeem (w http.ResponseWriter, r *http.Request){
	...
	if(e1!=nil || e2!=nil ){fmt.Fprint(w, "Illformated request"); return}
...
if(sig!=signature){
	...
	if(a1!=roll){fmt.Fprintf(w,"Not Authorised!");return}
...
sqlStatement := `SELECT pass FROM User WHERE id=$1;`
	...
	if(hi==0){fmt.Fprintf(w, "You do not EXIST ! xD"); return}
...
reci:=1
	...
	if(hi==0){fmt.Fprintf(w, "Item does not Exist !"); return}
	if(reci!=0){fmt.Fprintf(w, "Item already redeemed !"); return}
